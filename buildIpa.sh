...
if [ -f "$ENV_FILE" ]; then
    ...
    if [ "$ENVIRONMENT_PARAM" = "production" ]; then
        echo -e "\033[31mBuilding ipa for $ENVIRONMENT_PARAM\033[0m"
    else
        echo -e "\033[32mBuilding ipa for $ENVIRONMENT_PARAM\033[0m"
    fi
else
...
flutter build ipa --release  --dart-define=ENVIRONMENT=$ENVIRONMENT_PARAM